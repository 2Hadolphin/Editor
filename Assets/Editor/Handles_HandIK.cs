...
public class Handles_HandIK : Editor
{
        ...
        while (enumerator.MoveNext())
        {
            ...
            if (GUILayout.Button("Edit"))
            {
                if (dictionary.TryGetValue(id, out var editData))
                {
                    ...
                    {
                        ID = id;
                        _m.StartPoint = editData.Value.StartPoint;
                        _m.RelayPoint = editData.Value.RelayPoint;
                        _m.StopPoint = editData.Value.StopPoint;

                        _m.StartRotation = editData.Value.StartRotation;
                        _m.RelayRotation = editData.Value.RelayRotation;
                        _m.StopRotation = editData.Value.StopRotation;
    ...
    private void DrawBezier()
    {
        ...
        var pos= tf.position;
        Handles.DrawBezier(tf.TransformVector(_m.StartPoint)+pos, tf.TransformVector(_m.StopPoint) +pos, tf.TransformVector(_m.RelayPoint) + pos, tf.TransformVector(_m.RelayPoint) + pos, Color.white, null, 1f);
    }
    ...
    private void PositionHandle()
    {
        ...
        var 
        v = Handles.FreeMoveHandle(tf.position + tf.TransformVector(_m.StartPoint),
                                  Quaternion.identity,0.07f,Vector3.one,Handles.SphereHandleCap);
        _m.StartPoint = tf.InverseTransformVector(v - tf.position);
   

        Handles.color = Color.red;
        v = Handles.FreeMoveHandle(tf.position + tf.TransformVector(_m.StopPoint),
                                  Quaternion.identity, 0.07f, Vector3.one, Handles.SphereHandleCap);
        _m.StopPoint = tf.InverseTransformVector(v - tf.position);

        Handles.color = Color.yellow;
        v = Handles.FreeMoveHandle(tf.position + tf.TransformVector(_m.RelayPoint),
                                  Quaternion.identity, 0.07f, Vector3.one, Handles.SphereHandleCap);
    ...
    private static void PositionHandle_Axis()
    {
        ...
        Debug.Log(_m+""+tf);
        var 
        v = Handles.DoPositionHandle(tf.position + tf.TransformVector(_m.StartPoint),rot* q);
        _m.StartPoint = tf.InverseTransformVector(v - tf.position);

        v = Handles.DoPositionHandle(tf.position + tf.TransformVector(_m.StopPoint),
                                  rot * q);
        _m.StopPoint = tf.InverseTransformVector(v - tf.position);

        v = Handles.DoPositionHandle(tf.position + tf.TransformVector(_m.RelayPoint),
                                  rot *q);
        _m.RelayPoint = tf.InverseTransformVector(v - tf.position);
    }
    ...
    {
        Handles.color = Color.green;
        var r = Handles.DoRotationHandle(tf.rotation*_m.StartRotation, tf.position+tf.TransformVector(_m.StartPoint));
